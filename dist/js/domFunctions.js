...
const createCurrentConditionsDivs = (weatherObj, unit) => {
  ...
  const properDisc = toProperCase(weatherObj.current.weather[0].description);
  const disc = createElem("div", "disc", properDisc);
  ...
  return [icon, temp, disc, feels, maxTemp, minTemp, humidity, wind];
};
const createMainImgDiv = (icon, altText) => {
  const icondiv = createElem("div", "icon");
  icondiv.id = "icon";
  const faIcon = translateIconToFontAwesom(icon);
...
const translateIconToFontAwesom = (icon) => {
  ...
  switch (firstTwoChars) {
    ...
    case "10":
      ...
      break;
    case "11":
      i.classList.add("fas", "fa-poo-storn");
      break;
    case "13":
